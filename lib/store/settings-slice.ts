...
import type { User } from '@/types'; // UserProfile not directly used
...
export const createSettingsSlice = (
  ...
  return {
    ...
    user: null,
    speed_weight: 0.5, // Default value
    importance_weight: 0.5, // Default value
    auto_ranking: true, // Default value
    auto_subtasks: true, // Default value
    auto_tagging: true, // Default value
    ...
    loadInitialSettings: async (userId?: string) => {
      ...
      try {
        const { data, error } = await supabaseClient
          .from('user_settings')
          .select('gemini_api_key, theme, speed_weight, importance_weight, auto_ranking, auto_subtasks, auto_tagging')
        ...
        if (data) {
          set({
            ...
            speed_weight: data.speed_weight ?? 0.5,
            importance_weight: data.importance_weight ?? 0.5,
            auto_ranking: data.auto_ranking ?? true,
            auto_subtasks: data.auto_subtasks ?? true,
